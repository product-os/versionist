...
exports.parseGitLogYAMLOutput = (output, options = {}) => {
  ...
  return _.map(yaml.safeLoad(output), (commit) => {
    ...
    if (!options.parseFooterTags) {
      commitObj.body = options.bodyParser(commit.body);
      return commitObj;
    }
    ...
    commitObj.body = options.bodyParser(commitDescription.body.join('\n'));
    commitObj.footer = tags.parseFooterTagLines(commitDescription.footer, {
