 ...
 * @param {Boolean} lowerCaseFooterTags - lowercase footer tag keys
 ...
 * > baz
 */
exports.parseFooterTagLines = (footerTagLines, lowerCaseFooterTags) => {
  return _.chain(footerTagLines)
    ...
    .map((tagLine) => {
      ...
      finalTags.push([ tag.key, tag.value ]);
      if (lowerCaseFooterTags === true) {
        const loweredKey = tag.key.toLowerCase();
        if (loweredKey !== tag.key) {
          finalTags.push([ loweredKey, tag.value ]);
        }
      }
      return finalTags;
    })
