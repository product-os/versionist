...
const attachNestedChangelog = (upstreams, commit, callback) => {
  async.each(upstreams, (upstream, cb) => {
    const regexp = new RegExp(`Update ${upstream.pattern} from (\\S+) to (\\S+)`);
    const match = commit.body.match(regexp);

    if (match) {
      const currentVersion = match[1];
      const targetVersion = match[2];
      return getNestedChangeLog(upstream, commit, currentVersion, targetVersion, (err, nestedCommit) => {
        if (err) {
          return callback(err);
        }

        // TODO exit early once we found a match
        commit = nestedCommit;
        return cb(null);
      });
    }
