...
exports.generateChangelog = (commits, options = {}, callback) => {
  ...
  options.transformTemplateDataAsync(templateData, (error, transformedTemplate) => {
    ...
    const parsableChangelogPath = path.join(parsableChangelogFolder, parsableChangelogName);
    if (fs.existsSync(parsableChangelogFolder)) {
      let changelog = [];
      try {
        changelog = yaml.safeLoad(fs.readFileSync(parsableChangelogPath, 'utf8'));
      } catch (e) {
        // If no parsableChangelog exists, assume its empty
      }
