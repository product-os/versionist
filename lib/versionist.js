 ...
 * });
 */
exports.generateChangelog = (commits, options = {}, callback) => {
  ...
  options.transformTemplateDataAsync(templateData, (error, transformedTemplate) => {
    if (error) {
      return callback(error);
    }
    ...
    if (fs.existsSync(parsableChangelogPath)) {
      const changelog = yaml.safeLoad(fs.readFileSync(parsableChangelogPath, 'utf8'));
      changelog.unshift(transformedTemplate);
      fs.writeFileSync(parsableChangelogPath, yaml.safeDump(changelog));
    }

    return callback(null, template.render(options.template, transformedTemplate));
  });
};
 ...
 * @param {Object} opts - a dictionary of key-value pairs with a special
 *  cmd/command key which can be used to define the command (eg. get version)
 * @param {Function} callback - a callback (err, result) for users
 * @returns {undefined}
 */
exports.runWithOpts = (opts, callback) => {
  return exports.runWithParsedArgs(exports.parsedArgsFromOpts(opts), callback);
};
 ...
 *  `parsedArgsFromOpts()`)
 * @param {Function} rootCallback - a callback (err, result) for users (so named
 *  to avoid name collision with the callbacks of waterfall functions inside)
 * @returns {undefined}
 *
 */
exports.runWithParsedArgs = (parsedArgs, rootCallback) => {

  const stopError = 'E_STOP';

  const result = (obj) => {
    rootCallback(null, obj);
  ...
  async.waterfall([
    ...
    (currentVersion, documentedVersions, history, callback) => {
      ...
      // next version
      return exports.generateChangelog(history, {
        ...
        transformTemplateDataAsync: parsedArgs.configuration.transformTemplateDataAsync,
        version: version
      }, (error, entry) => {
        if (error) {
          return callback(error);
